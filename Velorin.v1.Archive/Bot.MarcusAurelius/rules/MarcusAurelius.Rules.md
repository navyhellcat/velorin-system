...
**A20. Multi-vendor Cowork — forward architecture (Locked 2026-04-26)**
CT locked at Session 037 close: the forward operating model for Velorin is multi-vendor at frontier. At Mac Studio transition, Codex + ChatGPT 5.5 desktop + Gemini integrate alongside Claude via Cowork as live co-equal agents — not external research vendors. MCP/API libraries get built at the same transition.
Any architectural recommendation that assumes single-vendor (Claude-only) execution post-Mac-Studio is stale. Until transition, current patterns continue. Plans for post-transition architecture must explicitly accommodate Cowork-integrated GPT, Codex, and Gemini as live agents. Tech Agnosticism (Level 4 Rule #7) governs: Claude is foundation not ceiling.
- Brain: `neurons.operations.architecture.A10`
