...
**Architectural framing (CT-clarified 2026-04-26):** When Cowork-orchestrated Claude faces a task, the program scores which external specialist system (Gemini Deep Research / Deep Think, Codex desktop, ChatGPT 5.5 desktop, Google photo/video tools, open-source tools, future persona-derived agents from FW-009, etc.) best matches the task — and Claude **activates it as if a direct sub-agent**. Output: "Claude activates System X for this task." The protocol underneath (MCP, API, A2A, CLI, desktop hand-off, browser automation) is implementation detail per-integration, not architecturally load-bearing — whatever works for each system is fine. The math is unchanged; the framing is functional sub-agent-activation, mechanism-agnostic.
...
**Architectural framing (CT-clarified 2026-04-26):** "Broker" is a sub-agent-activation pattern. Cowork-Claude activates a high-authority external specialist (typically a different vendor than the two whose outputs conflict — e.g., if GPT 5.5 and Gemini disagree, Claude activates a Codex run for adversarial-review on the disputed output, OR activates Gemini Deep Think for heavy reasoning over the conflict). The math (Ξ gate, orthogonality, authority_tier) is unchanged from Erdős's derivation; the activation mechanism (CLI, API, desktop hand-off, MCP, etc.) is per-integration implementation detail.
...
- Replace any rigid protocol-prescription framing ("must use A2A," "must use MCP exclusively," "decentralized agent swarm," "hub-and-spoke worker pattern") with **sub-agent-activation pattern** (CT-clarified 2026-04-26): Claude activates external specialist systems as if they were direct sub-agents; the underlying protocol per integration (MCP, API, A2A, CLI, desktop hand-off, browser automation) is implementation detail that gets built when needed. Function over protocol. Per Velorin.Principles.md Principle 8.
