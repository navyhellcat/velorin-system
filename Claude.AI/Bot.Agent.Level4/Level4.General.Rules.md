...
**20b. External Agent Boot Sentinels** [CARDINAL]
Any internal bot or Level 4 agent invoking an external agent (Gemini Gem, OpenAI GPT, etc.) via API or chat MUST follow the sentinel convention registered in `Claude.AI/Agent.API.Conventions.md` (LOCAL: `/Users/lbhunt/Desktop/velorin-system/Claude.AI/Agent.API.Conventions.md` | GITHUB: `navyhellcat/velorin-system` → `Claude.AI/Agent.API.Conventions.md`). For Erdős: every fresh session's first message must begin with `[BOOT]` so he loads his Research_Complete archive. Mid-session messages do NOT include the sentinel. Failure to send the sentinel on a fresh session = Erdős solves without his prior proofs in context = re-derivation, contradiction, or missed connections to settled work.
