...
| Data | Primary | Backup |
|---|---|---|
| Velorin Drive docs | Google Drive (cloud) | Drive desktop sync on Machine 1 (local) |
| Model weights | Drive 2 (Machine 2) | Drive 3 HDD archive |
| Code repos | GitHub | Local clone on Machine 2 Drive 1 |
| API keys | Local config files | Encrypted private document (offline) |
| Session logs | Drive (cloud) | Local archive on Machine 2 Drive 3 |

---

## Immediate Actions on New Machine Setup

1. Set `OLLAMA_MODELS` environment variable to point to Drive 2 path before downloading any models
2. Set up Google Drive Desktop Sync on Machine 1 immediately after OS install
3. Clone all Velorin repos to Drive 1 on Machine 2
4. Create folder structure on Drive 2 and Drive 3 before first use
